...
mkfs:
	...
	sudo mkfs.lustre --ost --mgsnode=$(IP)@tcp --index=0 --fsname=lustrefs /dev/loop32
...
.PHONY: all check_ip build_images configure mkfs mount mount_mgt mount_mdt mount_ost mount_client remove
